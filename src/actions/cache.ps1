...
function Get-CacheFiles {
    try {
        if (Test-DirectoryNotExists -path $Global:PVMConfig.paths.directories.cache) {
            return $null
        }

        $files = Get-ChildItemWrapper -path $Global:PVMConfig.paths.directories.cache -filter '*.json'
